...
describe('skill data access', () => {
    ...
    test('lists skills and public DTOs with category arrays', async () => {
        ...
        await expect(listSkillDtos(mock.db)).resolves.toEqual([{ id: 1, name: 'React', categories: [{ id: 10, name: 'Frontend' }, { id: 11, name: 'UI' }], featured: true }]);
    ...
    test('creates a skill and category assignments in one batch', async () => {
        ...
        const skill = await createSkill(mock.db, { name: 'TypeScript', category: '', featured: true }, [10]);

        expect(mock.prepare).toHaveBeenCalledWith('INSERT INTO skills (id, name, category, featured) VALUES (?, ?, ?, ?)');
        ...
        expect(skill).toEqual({ id: expect.any(Number), name: 'TypeScript', category: 'Language', featured: true, created_at: expect.any(String) });
    });
    ...
    test('updates fields and replaces categories', async () => {
        ...
        const result = await updateSkill(mock.db, 1, { name: 'React Updated' }, [11]);

        expect(result).toEqual(updatedSkill);
        expect(mock.prepare).toHaveBeenCalledWith('UPDATE skills SET name = ?, category = ? WHERE id = ?');
