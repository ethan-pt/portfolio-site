...
async function listProjectDtoRows(db: D1Database): Promise<AdminProjectDto[]> {
    ...
    const { results: projectSkillRows } = await db.prepare(
        `SELECT project_skills.project_id, skills.id AS skill_id, skills.name AS skill_name, skills.icon_url AS skill_icon_url, skills.featured AS skill_featured
         FROM project_skills
    ...
    for (const row of projectSkillRows) {
        ...
        skills.push({
            id: row.skill_id,
            name: row.skill_name,
            icon_url: row.skill_icon_url ?? null,
